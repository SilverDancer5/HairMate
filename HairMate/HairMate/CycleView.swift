...
//    var height:CGFloat?
    ...
    init(frame: CGRect,imageNames:[String],timeInterval:Double=2,pageControl:UIPageControl?=nil) {
        ...
        setupCollectionView(cellWidth: self.bounds.size.width, Spacing: 0)
        setupTimer()
    }
    ...
    //设置collectionView
    func  setupCollectionView(cellWidth: CGFloat, Spacing: CGFloat){
        let flowLayout = UICollectionViewFlowLayout()
        flowLayout.scrollDirection = .horizontal
        flowLayout.minimumLineSpacing = Spacing
        flowLayout.minimumInteritemSpacing = Spacing
//        flowLayout.itemSize = self.bounds.size
        flowLayout.itemSize = CGSize(width: cellWidth, height: self.bounds.size.height)
